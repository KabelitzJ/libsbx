...
namespace Demo
{
  public class CameraController_OLD : Behavior
  {
    ...
    {
      HandleKeyboardMovement(Time.DeltaTime);
      HandleMouseOrbit(Time.DeltaTime);
      HandleKeyRotation(Time.DeltaTime);
      HandleZoom(Time.DeltaTime);

      tilt.Update(Time.DeltaTime, 90f);
      zoom.Update(Time.DeltaTime, 10f);
      target.Y = 2.0f;
