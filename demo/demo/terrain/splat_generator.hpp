...
inline auto compute_cell_splat(const heightmap& height_map, std::int32_t cell_x, std::int32_t cell_z, const splat_generation_params& params) -> splat_weights {
  ...
  // Perturb the effective depth with noise to break contour lines at the shoreline
  auto depth_below_sea = terrain_constants::sea_level - average_height;
  auto effective_depth = depth_below_sea - (variety_noise + moisture_noise * 0.1f) * 3.0f;
  ...
  if (effective_depth > 1.0f) {
    // Deep underwater: sand/mud blend based on depth + moisture
    auto depth_factor = std::clamp((effective_depth - 1.0f) / 4.0f, 0.0f, 1.0f);
    auto moisture_factor = std::clamp(moisture_noise * 0.5f + 0.5f, 0.0f, 1.0f);
    sand = depth_factor + (1.0f - depth_factor) * (1.0f - moisture_factor);
    mud = (1.0f - depth_factor) * moisture_factor;
  } else if (effective_depth > -3.0f) {
    // Shore transition: wide blend from mud/sand through to grass
    auto shore_factor = std::clamp((1.0f - effective_depth) / 4.0f, 0.0f, 1.0f);
    auto moisture_factor = std::clamp(moisture_noise * 0.5f + 0.5f, 0.0f, 1.0f);
    mud = (1.0f - shore_factor) * moisture_factor * 0.6f;
    sand = (1.0f - shore_factor) * (1.0f - moisture_factor) * 0.8f;
    dirt = (1.0f - shore_factor) * 0.3f;
    grass = shore_factor;
  ...
  return splat_weights::from_floats(grass, dirt, rock, sand, snow, mud);
}
...
inline auto generate_splat_map(const heightmap& height_map, const splat_generation_params& params = {}) -> std::vector<splat_weights> {
  ...
  for (auto cell_z = 0u; cell_z < cell_count_z; ++cell_z) {
    for (auto cell_x = 0u; cell_x < cell_count_x; ++cell_x) {
      weights[cell_z * cell_count_x + cell_x] = compute_cell_splat(height_map, static_cast<std::int32_t>(cell_x), static_cast<std::int32_t>(cell_z), params);
    }
  }
...
inline auto regenerate_splat_region(std::vector<splat_weights>& weights, const heightmap& height_map, std::int32_t min_x, std::int32_t min_z, std::int32_t max_x, std::int32_t max_z, const splat_generation_params& params = {}) -> void {
  ...
  for (auto cell_z = min_z; cell_z <= max_z; ++cell_z) {
    for (auto cell_x = min_x; cell_x <= max_x; ++cell_x) {
      weights[static_cast<std::uint32_t>(cell_z) * cell_count_x + static_cast<std::uint32_t>(cell_x)] = compute_cell_splat(height_map, cell_x, cell_z, params);
    }
  }
}
