...
: sbx::graphics::subrenderer{},
  ...
  for (auto i = 0u; i < terrain_layer_count; ++i) {
    _layer_textures[i] = graphics_module.add_resource<sbx::graphics::image2d>(layer_paths[i], sbx::graphics::format::r8g8b8a8_srgb, sbx::graphics::filter::linear, sbx::graphics::address_mode::repeat, true, true);
    _terrain_images.push_back(_layer_textures[i]);
  }

  auto normal_paths = std::array<std::filesystem::path, terrain_layer_count>{
    "res://textures/terrain/grass/normal.jpg",
    "res://textures/terrain/dirt/normal.jpg",
    "res://textures/terrain/rock/normal.jpg",
    "res://textures/terrain/sand/normal.jpg",
    "res://textures/terrain/snow/normal.jpg",
    "res://textures/terrain/mud/normal.jpg",
  };

  for (auto i = 0u; i < terrain_layer_count; ++i) {
    _layer_textures[terrain_layer_count + i] = graphics_module.add_resource<sbx::graphics::image2d>(normal_paths[i], sbx::graphics::format::r8g8b8a8_unorm, sbx::graphics::filter::linear, sbx::graphics::address_mode::repeat, true, true);
    _terrain_images.push_back(_layer_textures[terrain_layer_count + i]);
  }
...
auto terrain_subrenderer::render(sbx::graphics::command_buffer& command_buffer) -> void {
  ...
  auto map_width = terrain_module.world_extent_x();
  auto map_height = terrain_module.world_extent_z();
  ...
  for (auto ring = 0u; ring < clipmap_ring_count; ++ring) {
    ...
    _push_handler.push("cells_x", terrain_module.cells_x());
    _push_handler.push("cells_z", terrain_module.cells_z());
