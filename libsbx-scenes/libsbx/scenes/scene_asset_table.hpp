...
#ifndef LIBSBX_SCENES_SCENE_ASSET_TABLE_HPP_
#define LIBSBX_SCENES_SCENE_ASSET_TABLE_HPP_
...
public:
  ...
  auto insert(const utility::hashed_string& name, const Handle& handle, assets::asset_metadata metadata) -> void {
    _by_name.emplace(name, handle);
    _metadata.emplace(handle, std::move(metadata));
  }

  auto metadata(const Handle& handle) const -> const assets::asset_metadata& {
...
private:
  ...
  std::unordered_map<Handle, assets::asset_metadata> _metadata;

}; // class asset_table

class scene_asset_table {
...
public:
  ...
  auto add_image(const utility::hashed_string& name, const std::filesystem::path& path, Args&&... args) -> void {
    ...
    _images.insert(name, id, assets::asset_metadata{path, name.str(), "image", "disk"});
  }
  ...
  auto add_cube_image(const utility::hashed_string& name, const std::filesystem::path& path, Args&&... args) -> void {
    ...
    _cube_images.insert(name, id, assets::asset_metadata{path, name.str(), "cube_image", "disk"});
  }
  ...
  auto add_mesh(const utility::hashed_string& name, Args&&... args) -> void {
    ...
    _meshes.insert(name, id, assets::asset_metadata{"", name.str(), "mesh", "generated"});
  }
  ...
  auto add_mesh(const utility::hashed_string& name, const Path& path, Args&&... args) -> void {
    ...
    _meshes.insert(name, id, assets::asset_metadata{path, name.str(), "mesh", "disk"});
  }
  ...
  auto add_animation(const utility::hashed_string& name, Args&&... args) -> void {
    ...
    _animations.insert(name, id, assets::asset_metadata{"", name.str(), "animation", "generated"});
  }
  ...
  auto add_material(const utility::hashed_string& name, Args&&... args) -> Material& {
    ...
    _materials.insert(name, id, assets::asset_metadata{"", name.str(), "material", "dynamic"});
...
#endif // LIBSBX_SCENES_SCENE_ASSET_TABLE_HPP_
