...
: _set{set},
  ...
  auto variable_info = VkDescriptorSetVariableDescriptorCountAllocateInfo{};
  ...
  if (variable_descriptor_index >= 0) {
    ...
    variable_info.pDescriptorCounts = counts.data();

    descriptor_set_allocate_info.pNext = &variable_info;
  }
