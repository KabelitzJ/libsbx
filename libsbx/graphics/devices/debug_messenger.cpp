...
static auto _message_type_to_string(const VkDebugUtilsMessageTypeFlagsEXT message_type) -> std::string_view {
  switch (message_type) {
...
VKAPI_ATTR auto VKAPI_CALL debug_messenger::_debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT message_severity, VkDebugUtilsMessageTypeFlagsEXT message_type, const VkDebugUtilsMessengerCallbackDataEXT* callback_data, [[maybe_unused]] void* user_data) -> VkBool32 {
  if (message_severity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
    utility::logger<"graphics">::warn("{}: {}", _message_type_to_string(message_type), callback_data->pMessage);
  } else if (message_severity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
    utility::logger<"graphics">::error("{}: {}", _message_type_to_string(message_type), callback_data->pMessage);
    std::terminate();
  }
