...
: _present_mode{VK_PRESENT_MODE_FIFO_KHR},
  ...
  utility::logger<"graphics">::debug("Created swapchain ({}x{}) with {} images", _extent.width, _extent.height, desired_image_count);
	...
	validate(vkGetSwapchainImagesKHR(logical_device, _handle, &_image_count, nullptr), "vkGetSwapchainImagesKHR");

	_images.resize(_image_count);
  _image_views.resize(_image_count);

	validate(vkGetSwapchainImagesKHR(logical_device, _handle, &_image_count, _images.data()), "vkGetSwapchainImagesKHR");
  ...
  logical_device.set_debug_name(_handle, "Swapchain");

  for (std::uint32_t i = 0; i < _image_count; ++i) {
    logical_device.set_debug_name(_images.at(i), fmt::format("Swapchain Image {}", i));
    logical_device.set_debug_name(_image_views.at(i), fmt::format("Swapchain Image View {}", i));
  }
}
...
auto swapchain::_create_image_view(const VkImage& image, VkFormat format, VkImageAspectFlags aspect, VkImageView& image_view) -> void {
  ...
  validate(vkCreateImageView(logical_device, &image_view_create_info, nullptr, &image_view), "vkCreateImageView");
}
