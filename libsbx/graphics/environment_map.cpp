...
auto environment_map::_generate_brdf(const std::uint32_t size) -> void {
...
auto environment_map::_generate_irradiance(const std::uint32_t size) -> void {
  constexpr auto threads_per_group = std::uint32_t{16};
  ...
  auto& skybox = graphics_module.get_resource<sbx::graphics::cube_image>(_skybox);
...
auto environment_map::_generate_prefiltered(uint32_t size) -> void {
  constexpr auto threads_per_group = std::uint32_t{16};
