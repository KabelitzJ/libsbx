...
auto static_mesh_material_subrenderer::render(graphics::command_buffer& command_buffer) -> void {
  ...
  for (auto& [key, data] : draw_list.ranges(_bucket)) {
    ...
    for (const auto& ref : data.ranges) {
      auto& mesh = assets_module.get_loaded<models::mesh>(ref.mesh_id);
      ...
      auto command_offset = culled ? (ref.range.offset - data.commands_offset) : ref.range.offset;
      command_buffer.draw_indexed_indirect(draw_commands_buffer, command_offset, ref.range.count);
    }
  }
}
