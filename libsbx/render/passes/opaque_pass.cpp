...
auto opaque_pass::_resolve_graph_pipeline(const assets::shader_graph_handle& graph, bool is_double_sided) -> memory::observer_ptr<graphics::graphics_pipeline> {
  ...
  try {
    ...
    return _make_pipeline(shader, is_double_sided ? graphics::cull_mode::none : graphics::cull_mode::back, fmt::format("Mesh Opaque Graph {}", assets::shader_graph_generated_name(graph->id())));
