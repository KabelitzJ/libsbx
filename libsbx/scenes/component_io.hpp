...
struct component_io {
  ...
  std::string name;
  std::function<void(YAML::Emitter&, scene_graph&, asset_registry&, const node)> save;
  std::function<void(const YAML::Node&, scene_graph&, asset_registry&, const node)> load;
...
public:
  ...
  auto register_component(const std::string& name, Save&& save, Load&& load) -> void {
    ...
    _by_id[id] = component_io{
      .name = name,
      .save = [s = std::forward<Save>(save)](YAML::Emitter& emitter, scene_graph& graph, asset_registry& registry, const node node) -> void {
        const auto& component = graph.get_component<Type>(node);

        std::invoke(s, emitter, graph, registry, component);
      },
      .load = [l = std::forward<Load>(load)](const YAML::Node& yaml, scene_graph& graph, asset_registry& registry, const node node) -> void {
        graph.add_or_update_component<Type>(node, std::invoke(l, yaml, graph, registry));
      }
    };
  }
