...
auto script_compiler::_write_ide_project(const std::filesystem::path& assets_directory, const std::filesystem::path& ide_output_directory, const std::filesystem::path& core_assembly_path) -> void {
  ...
  const auto content = fmt::format(
    ...
    "         Must be set before Sdk.props is imported below, or restore still writes obj/ here. -->\n"
    "    <BaseOutputPath>{0}</BaseOutputPath>\n"
    "    <BaseIntermediateOutputPath>{1}</BaseIntermediateOutputPath>\n"
    "    <MSBuildProjectExtensionsPath>{1}</MSBuildProjectExtensionsPath>\n"
    "  </PropertyGroup>\n"
    "\n"
