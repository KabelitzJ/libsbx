 ...
 *
 * Single-threaded by construction -- every call site is on the main thread (the same assumption
 * scoped_timer's own doc comment already makes), so there is no locking here.
 */
